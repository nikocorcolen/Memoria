...
\begin{itemize}
\item El objetivo es automatizar el riego de un jardín utilizando un sistema inteligente de bajo costo para el uso eficiente del agua.
\end{itemize}

\paragraph{Objetivos específicos}
\begin{itemize}
\item Diseñar un prototipo funcional de la idea a desarrollar.
\item Generar un modelo de riego con los datos obtenidos, para utilizar eficientemente el agua.
\item Diseñar un protoripo de aplicación web y/o móvil que les permita a los usuarios monitorear y controlar el riego.
\item Alertar a los usuarios en caso de que existan condiciones climaticas que generen escarcha y puedan dañar el cesped.
\end{itemize}

\section{Alcances}
\begin{itemize}
\item Este trabajo se limita a el control y monitoreo del riego, el sistema de riego (cañerías, aspersores, etc.) deben estar instalados.
\item El modelo de riego será básico y se podrá mejorar con más tiempo y personas especialistas en agricultura.
\item En el monitoreo los usuario podrán ver la humedad y la cantidad de agua utilizada en el riego.
\item En el control, los usuarios podrán activar y desactivar el sistema de riego.
\item Se diseñará una aplicación web y/o móvil que permita a los usuarios visualizar la humedad del suelo y la cantidad de agua utilizada, y además tendra la funciona de activar y desactivar el riego.
\end{itemize}
...
Es por ello, que es muy importante y de suma urgencia idear nuevas técnicas de riego sustentables en las que se obtenga el mayor provecho del agua que permita mantener el nivel de producción agricula en Chile o incluso aumentarlo.
...
A continuación se muestran algunas imágenes de un prototipo básico del dispositivo que tiene algunas funcionalidades.

La Figura \ref{prot2} se peude ver el micro computador funcionando, el cual esta conectado a Internet por cable de red y el sensor de humedad conectado a la interfaz GPIO. De fondo en la pantalla se esta ejecutando el programa que lee los datos del sensor de humedad, como en esta caso la tierra se encuentra seca, el programa muestra un 1, que significa que se debe regar.

\begin{figure}[h]
\centering
\includegraphics[width=10cm]{prot2.jpg}
\caption{Prototipo funcionando con tierra seca}
\label{prot2}
\end{figure}

\newpage

En la Figura \ref{prot2} la diferencia es que la tierra esta húmeda, por lo que el programa muestra un 0 que significa que no se debe regar.

\begin{figure}[h]
\centering
\includegraphics[width=10cm]{prot1.jpg}
\caption{Prototipo funcionando con tierra húmeda}
\label{prot2}
\end{figure}

La Figura \ref{climatologicos} muestra los datos entregados por el servicio web meteorológico, entregándole como parámetro las coordenadas de la posición con un GPS.

\begin{figure}[h]
\centering
\includegraphics[width=13cm]{climaGps.png}
\caption{Datos climatológicos}
\label{climatologicos}
\end{figure}
