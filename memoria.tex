...
La programación del riego, es la planificación de cuándo y qué cantidad de agua aplicar con el fin de mantener el crecimiento saludable del césped. Es una práctica de gestión diaria esencial.
...
\item Tipo de suelo: indica el tipo de suelo en el que el dispositivo esta siendo utilizado.
\item Humedad del suelo: indica la humedad del suelo utilizando sensores de humedad.
\end{itemize}

Existe más información que puede ser incluida para mejorar la calidad del riego aplicado al suelo, pero para en este trabajo solo se considera las variables mencionadas anteriormente.
...
volatile int  frecuencia\_sensor;  \textit{// Mide los pulsos de sensor de flujo}
...
void loop() \{
	...
	humedad\_sensor = analogRead(pinHumedad);  \textit{// Entrada analógica, sensor de humedad}
...
\begin{figure}[h]
\centering
\includegraphics[width=10cm]{conexiones.jpg}
...
\begin{figure}[h]
\centering
\includegraphics[width=10cm]{lsgps.png}
...
\item En el archivo creado anteriormente copiar el código del siguiente link \url{https://gist.github.com/wolfg1969/4653340#file-gpsddata-py}
\item Ejecutar \textit{sudo python gps.py} con esto obtenemos como resultado lo que se aprecia en la Figura \ref{datosgps}:
\begin{figure}[h]
\centering
\includegraphics[width=10cm]{datosgps.png}
\caption{Datos GPS}
\label{datosgps}
\end{figure}
